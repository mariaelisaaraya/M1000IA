...
images_folder = 'merged_images'
images = load_images_from_folder(images_folder)

# Cargar el archivo CSV de metadatos
metadata_file = 'metadatos_actualizados_limpios_normalizados.csv'
...
# Función de guardado del modelo completo y los parámetros
def save_model_and_params(model, optimizer, epoch, loss, model_filename='modelo_completo.pth'):
    ...
    torch.save({
        ...
        'epoch': epoch,
        'loss': loss,
        'model_architecture': model  # Incluye la arquitectura del modelo
    }, model_filename)

# Función de entrenamiento con ajuste de umbral
def train_model(model, train_loader, val_loader, num_epochs=30, threshold=0.5):
    best_f1 = 0.0  # Mejor F1 para el guardado del modelo
    for epoch in range(num_epochs):
        ...
        y_true, y_pred = evaluate_model(model, val_loader, threshold)
        ...
        if f1 > best_f1:
            best_f1 = f1
            save_model_and_params(model, optimizer, epoch, epoch_loss / len(train_loader))
            print("Mejor modelo completo guardado.")
