...
df = pd.read_csv('metadatos_actualizados_limpios_normalizados.csv')
...
image_dir = 'merged_images'  # Directorio donde se encuentran las imágenes
...
etiquetas = pd.get_dummies(df['classification']).values  # Convertir etiquetas a one-hot encoding
et=pd.get_dummies(df['classification'])
...
with h5py.File('datasetMerged2Etiquetas.h5', 'w') as hdf:
